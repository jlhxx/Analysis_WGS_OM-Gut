...
otu_mat <- read_excel("gut_saliva_16.xlsx", sheet = "gut_otu")
tax_mat<- read_excel("gut_saliva_16.xlsx", sheet = "gut_taxonomy")
samples_df <- read_excel("gut_saliva_16.xlsx", sheet = "gut_sample")
...
taxa_to_remove <- taxa_names(gut)[species_names %in% grep("^s__GGB", species_names, value = TRUE)]
...
sample_count <- nsamples(gut_filtered2)
...
gut_filtered_prevalence <- prune_taxa(apply(otu_table(gut_filtered2), 1, prevalence_filter), gut_filtered2)

# Convert to relative abundance
gut_rel_abundance <- transform_sample_counts(gut_filtered_prevalence, function(x) x / sum(x))
...
gut_filtered_abundance <- prune_taxa(apply(otu_table(gut_rel_abundance), 1, abundance_filter), gut_filtered_prevalence)
...
readcount = data.table(as(sample_data(gut_filtered_abundance), "data.frame"),
                       TotalReads = sample_sums(gut_filtered_abundance), 
                       keep.rownames = TRUE)
...
ps0.rar <- gut_filtered_abundance
